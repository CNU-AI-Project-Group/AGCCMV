...
# from vichlerequest import tyre, errortyre
from carcontrol import tyre

session = requests.Session()
# ---------------------------设置参数------------------------

weights = r'C:\Projects\AI\yolov5-swarm\besthd&ld.pt'  # 模型存放的位置
# weights = r"C:\Projects\AI\yolov5-swarm\CF_AI\cfbest.pt"
window_size = (590, 220, 1330, 860)  # 需要截屏的位置(640, 220, 1280, 860)
re_x, re_y = 1600, 1600  # 1.cv2窗口大小
img_size = 640  # 输入到yolo5中的模型尺寸

conf_thres = 0.5  # NMS的置信度过滤
    ...
    for i, det in enumerate(pred):  # detections per image  ----  依次取出一个pred里的单个目标
        ...
        if len(det):  # 如果det不为零
            ...
            for *xyxy, conf, cls in reversed(det):
                ...
                if label:
                    with open(f"./datasets/coco12/labels/train2017/{coutt}.txt", 'w') as f:
                        lin = ' '.join([str(i) for i in xywh])
                        f.write(f'0 {lin}')
                        f.close()
        ...
        if len(aims):
            ...
            if plot:
                ...
                for i, det in enumerate(aims):
                    ...
                    color = (0, 255, 0)  # RGB
                    if label:
                        cv2.imwrite(f'./datasets/coco12/images/train2017/{coutt}.jpg', img0)
                    cv2.rectangle(img0, top_left, bottom_right, color, thickness=3)  # 画框
                    cr1, cr2 = (int(xyxy[0]), int(xyxy[1])), (int(xyxy[2]), int(xyxy[3]))
                    if label:
                        cv2.imwrite(f'./datasets/coco12/images/2017/{coutt}.jpg', img0)
                    wx = (abs(int(cr1[0]) + int(cr2[0]))) // 2
                    wy = (abs(int(cr1[1]) + int(cr2[1]))) // 2  # 目标中心坐标（wx,wy）
                    center = (int(img0.shape[1]) // 2, int(img0.shape[0]) // 2)  # 屏幕中心 窗宽和窗高

                    if coutt // 1:
                        print(center, wx, wy, (cr2[0] - cr1[0], cr2[1] - cr1[1]))
                        tyre(center, wx, wy, (cr2[0] - cr1[0], cr2[1] - cr1[1]),91)
            else:
                pass
                # r = session.get(f'http://192.168.31.{ipp}/s')
        else:
            pass
            # r = session.get(f'http://192.168.31.91/s')
